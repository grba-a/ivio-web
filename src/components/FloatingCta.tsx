...
export function FloatingCta() {
  ...
  return (
    // Fixed positioning lives on this outer wrapper, which never itself
    // transitions or transforms. WebKit can detach a `position: fixed`
    // element from the viewport mid-scroll when a transform on that same
    // element is actively transitioning - nesting the animated part inside
    // a static fixed shell keeps the pill actually pinned to the corner.
    <div
    ...
    >
      <a
        ...
        aria-hidden={!show}
        tabIndex={show ? 0 : -1}
        className="inline-flex min-h-12 items-center gap-2.5 whitespace-nowrap rounded-full bg-amber px-6 text-[15px] font-medium text-[#101f26] shadow-[0_10px_36px_-10px_rgba(4,22,30,0.65)] transition-all duration-300"
        style={{
          opacity: show ? 1 : 0,
